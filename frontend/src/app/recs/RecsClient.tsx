...
export default function RecsClient() {
  ...
  const run = useCallback(
    async (queryOverride?: string) => {
      ...
      try {
        ...
        const data: { results?: Rec[] } = await res.json()
        setResults(data.results ?? [])
      } catch (e: unknown) {
        const msg = getErrorMessage(e, 'Failed to fetch recommendations')
        setError(msg === 'AbortError' ? 'Request timed out - please try again.' : msg)
      } finally {
        setLoading(false)
      }
    },
  ...
  useEffect(() => {
    if (qp && !q) {
      setQ(qp)
      // fire and forget; dependencies handled via useCallback
      void run(qp)
    }
  }, [qp, q, run])
    ...
    <PageShell>
      <div className="max-w-5xl mx-auto p-6 space-y-5">
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              />
              {searching && <div className="absolute right-2 top-2 text-xs text-muted">Searching…</div>}
            </div>
        ...
        <div className="flex gap-2">
          <Button size="lg" onClick={() => void run()} disabled={loading || (!q && !selected)}>
